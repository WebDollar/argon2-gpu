...
machines$id <-paste0(machines$type, '-', machines$name)
...
make_plots_bps <- function(type) {
  ...
  if (length(data_b$hashes_per_second) != c(0)) {
    ...
    save_graph(paste0(prefix, '-t_cost'),
               ggplot(data_b[data_b_f_m_cost,], aes(x=t_cost, y=blocks_per_second, group=Machine, colour=Machine)) +
                 geom_line() +
    ...
    save_graph(paste0(prefix, '-m_cost'),
               ggplot(data_b[data_b_f_t_cost,], aes(x=m_cost, y=blocks_per_second, group=Machine, colour=Machine)) +
                 geom_line() +
                 scale_x_log10() +
                 scale_y_continuous(labels=comma) +
    ...
    save_graph(paste0(prefix, '-lanes'),
               ggplot(data_b[data_b_f_t_cost & data_b_f_m_cost,], aes(x=lanes, y=blocks_per_second, group=Machine, colour=Machine)) +
                 geom_line() +
                 scale_y_continuous(labels=comma) +
...
make_plots_bpj <- function(type) {
  ...
  if (length(data_b$hashes_per_second) != c(0)) {
    ...
    save_graph(paste0(prefix, '-t_cost'),
               ggplot(data_b[data_b_f_m_cost,], aes(x=t_cost, y=blocks_per_joule, group=Machine, colour=Machine)) +
                 geom_line() +
                 scale_y_continuous(labels=comma) +
                 facet_grid(lanes~m_cost, labeller=label_both) +
    ...
    save_graph(paste0(prefix, '-m_cost'),
               ggplot(data_b[data_b_f_t_cost,], aes(x=m_cost, y=blocks_per_joule, group=Machine, colour=Machine)) +
                 geom_line() +
                 scale_x_log10() +
                 scale_y_continuous(labels=comma) +
                 facet_grid(t_cost~lanes, labeller=label_both) +
    ...
    save_graph(paste0(prefix, '-lanes'),
               ggplot(data_b[data_b_f_t_cost & data_b_f_m_cost,], aes(x=lanes, y=blocks_per_joule, group=Machine, colour=Machine)) +
                 geom_line() +
                 scale_y_continuous(labels=comma) +
                 facet_grid(t_cost~m_cost, labeller=label_both) +

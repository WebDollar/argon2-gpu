...
make_plots_commits <- function(mode, kernel, type, precompute) {
  ...
  if (length(data_b$hashes_per_second) != c(0)) {
    ...
    save_graph(paste0(prefix, '-t_cost'),
               ggplot(data_b[data_b_f_m_cost,], aes(x=t_cost, y=blocks_per_second, group=Commit, colour=Commit)) +
                 geom_line() +
    ...
    save_graph(paste0(prefix, '-lanes'),
               ggplot(data_b[data_b_f_t_cost & data_b_f_m_cost,], aes(x=lanes, y=blocks_per_second, group=Commit, colour=Commit)) +
                 geom_line() +
                 scale_y_continuous(labels=comma) +
...
make_plots_types <- function(commit, mode, kernel) {
  ...
  if (length(data_b$hashes_per_second) != c(0)) {
    ...
    save_graph(paste0(prefix, '-t_cost'),
               ggplot(data_b[data_b_f_m_cost,], aes(x=t_cost, y=blocks_per_second, group=Variant, colour=Variant)) +
                 geom_line() +
                 scale_y_continuous(labels=comma) +
                 facet_grid(lanes~m_cost, labeller=label_both) +
                 xlab('t_cost') + ylab('Blocks per second'))
    ...
    save_graph(paste0(prefix, '-lanes'),
               ggplot(data_b[data_b_f_t_cost & data_b_f_m_cost,], aes(x=lanes, y=blocks_per_second, group=Variant, colour=Variant)) +
                 geom_line() +
                 scale_y_continuous(labels=comma) +
                 facet_grid(t_cost~m_cost, labeller=label_both) +
                 xlab('lanes') + ylab('Blocks per second'))
...
make_plots_versions <-  function(commit, mode, kernel, type, precompute) {
  ...
  if (length(data_b$hashes_per_second) != c(0)) {
    ...
    save_graph(paste0(prefix, '-t_cost'),
               ggplot(data_b[data_b_f_m_cost,], aes(x=t_cost, y=blocks_per_second, group=Version, colour=Version)) +
                 geom_line() +
                 scale_y_continuous(labels=comma) +
                 facet_grid(lanes~m_cost, labeller=label_both) +
                 xlab('t_cost') + ylab('Blocks per second'))
    ...
    save_graph(paste0(prefix, '-lanes'),
               ggplot(data_b[data_b_f_t_cost & data_b_f_m_cost,], aes(x=lanes, y=blocks_per_second, group=Version, colour=Version)) +
                 geom_line() +
                 scale_y_continuous(labels=comma) +
                 facet_grid(t_cost~m_cost, labeller=label_both) +
                 xlab('lanes') + ylab('Blocks per second'))
...
make_plots_kernels <-  function(commit, mode, version, type, precompute) {
  ...
  if (length(data_b$hashes_per_second) != c(0)) {
    ...
    save_graph(paste0(prefix, '-t_cost'),
               ggplot(data_b[data_b_f_m_cost,], aes(x=t_cost, y=blocks_per_second, group=Kernel.mode, colour=Kernel.mode)) +
                 geom_line() +
                 scale_y_continuous(labels=comma) +
                 facet_grid(lanes~m_cost, labeller=label_both) +
                 xlab('t_cost') + ylab('Blocks per second'))
    ...
    save_graph(paste0(prefix, '-lanes'),
               ggplot(data_b[data_b_f_t_cost & data_b_f_m_cost,], aes(x=lanes, y=blocks_per_second, group=Kernel.mode, colour=Kernel.mode)) +
                 geom_line() +
                 scale_y_continuous(labels=comma) +
                 facet_grid(t_cost~m_cost, labeller=label_both) +
                 xlab('lanes') + ylab('Blocks per second'))
...
make_plots_modes <-  function(commit, kernel, version, type, precompute) {
  ...
  if (length(data_b$hashes_per_second) != c(0)) {
    ...
    save_graph(paste0(prefix, '-t_cost'),
               ggplot(data_b[data_b_f_m_cost,], aes(x=t_cost, y=blocks_per_second, group=Mode, colour=Mode)) +
                 geom_line() +
                 scale_y_continuous(labels=comma) +
                 facet_grid(lanes~m_cost, labeller=label_both) +
                 xlab('t_cost') + ylab('Blocks per second'))
    ...
    save_graph(paste0(prefix, '-lanes'),
               ggplot(data_b[data_b_f_t_cost & data_b_f_m_cost,], aes(x=lanes, y=blocks_per_second, group=Mode, colour=Mode)) +
                 geom_line() +
                 scale_y_continuous(labels=comma) +
                 facet_grid(t_cost~m_cost, labeller=label_both) +
                 xlab('lanes') + ylab('Blocks per second'))
